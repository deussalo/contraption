...
import {createWorld,createBody,setMass,overlaps,connectionPoints,pathLength} from './physics.js';
export class Workshop{
  ...
  move(body,change){
    if(!this.editable(body))throw Error('This object is locked.');Object.assign(body,change);setMass(body);Object.assign(this.level.bodies.find(p=>p.id===body.id),designPart(body));this.world.cachedContacts=[];this.changed=true;
  }
  ...
  addConnection(kind,a,b,anchors,via=[]){
    ...
    if(kind==='belt'&&(![first,last].every(p=>['motor','pulley','conveyor'].includes(p.kind))||Math.hypot(first.x-last.x,first.y-last.y)>650))throw Error('Belts join wheels and conveyors within 650 units.');
    ...
    const joint={id:uid(),kind,a,b,...anchors,via,length:1};joint.length=Math.max(1,pathLength(connectionPoints(this.world,joint)));
