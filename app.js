...
let traySignature='';
function renderTray(){
  const puzzle=workshop.level.mode==='puzzle',liveLocked=puzzle&&workshop.running,entries=puzzle?workshop.level.inventory.map(e=>({kind:e.part.kind,part:e.part,stock:e.id,count:workshop.remaining(e)})):Object.keys(PARTS).map(kind=>({kind,part:PARTS[kind],count:null}));
  const signature=JSON.stringify([liveLocked,entries]);if(signature===traySignature)return;traySignature=signature;$('parts-grid').replaceChildren();$('bin-label').textContent=puzzle?entries.reduce((sum,e)=>sum+e.count,0):'∞';
  ...
  for(const entry of entries){const button=document.createElement('button');button.className='part-card';button.dataset.kind=entry.kind;const dimensions=PARTS[entry.kind].shape==='circle'?`Ø${Math.round(entry.part.w)}`:`${Math.round(entry.part.w)}×${Math.round(entry.part.h)}`,fixedSize=puzzle||!entry.part.resizable,description=`Drag ${dimensions} ${PARTS[entry.kind].name} onto the canvas${fixedSize?', fixed size':''}${entry.count===null?'':`, ${entry.count} left`}`;button.setAttribute('aria-label',liveLocked?'Pause the puzzle to place parts':description);button.title=liveLocked?'Pause the puzzle to place parts':`Drag ${dimensions} ${PARTS[entry.kind].name} onto the canvas${fixedSize?' · fixed size':''}`;button.disabled=liveLocked||entry.count===0;
...
async function toggleRun(){if(!workshop.running)gestures.cancel();workshop.running=!workshop.running;sync();if(workshop.running)try{await sound.unlock();}catch(error){sound.enabled=false;toast(error.message);}}
