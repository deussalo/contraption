...
function sync(){
  ...
  $('mode-name').textContent=editor?'Puzzle editor':puzzle?workshop.level.name:'Workshop';for(const id of ['undo','menu-undo'])$(id).disabled=!workshop.constructionEnabled()||!workshop.history.length;for(const id of ['redo','menu-redo'])$(id).disabled=!workshop.constructionEnabled()||!workshop.future.length;$('step').disabled=running||rewinding;$('rewind').disabled=!rewinding&&!timeline.canRewind;$('rewind').classList.toggle('active',rewinding);$('rewind').setAttribute('aria-pressed',String(rewinding));
...
let traySignature='';
function renderTray(){
  const puzzle=workshop.level.mode==='puzzle',attemptLocked=puzzle&&!workshop.constructionEnabled(),entries=puzzle?workshop.level.inventory.map(e=>({kind:e.part.kind,part:e.part,stock:e.id,count:workshop.remaining(e)})):Object.keys(PARTS).map(kind=>({kind,part:PARTS[kind],count:null}));
  const signature=JSON.stringify([attemptLocked,entries]);if(signature===traySignature)return;traySignature=signature;$('parts-grid').replaceChildren();$('bin-label').textContent=puzzle?entries.reduce((sum,e)=>sum+e.count,0):'∞';
  ...
  for(const entry of entries){const button=document.createElement('button');button.className='part-card';button.dataset.kind=entry.kind;const dimensions=PARTS[entry.kind].shape==='circle'?`Ø${Math.round(entry.part.w)}`:`${Math.round(entry.part.w)}×${Math.round(entry.part.h)}`,fixedSize=puzzle||!entry.part.resizable,description=`Drag ${dimensions} ${PARTS[entry.kind].name} onto the canvas${fixedSize?', fixed size':''}${entry.count===null?'':`, ${entry.count} left`}`;button.setAttribute('aria-label',attemptLocked?'Reset the puzzle to place parts':description);button.title=attemptLocked?'Reset the puzzle to place parts':`Drag ${dimensions} ${PARTS[entry.kind].name} onto the canvas${fixedSize?' · fixed size':''}`;button.disabled=attemptLocked||entry.count===0;
...
async function toggleRun(){if(!workshop.running){gestures.cancel();workshop.play();}else workshop.pause();sync();if(workshop.running)try{await sound.unlock();}catch(error){sound.enabled=false;toast(error.message);}}
function stepSimulation(dt=1/120){workshop.beginAttempt();const events=stepWorld(workshop.world,dt);timeline.record(workshop.world,workshop.revision);return events;}
function beginRewind(){gestures.cancel();timeline.ensure(workshop.world,workshop.revision);if(!timeline.start(workshop.world,workshop.revision))return;resumeAfterRewind=workshop.running;workshop.pause();particles=[];accumulator=0;sync();}
function endRewind(){if(!timeline.stop(workshop.world))return;if(resumeAfterRewind)workshop.play();resumeAfterRewind=false;workshop.changed=true;accumulator=0;sync();}
...
function goalDialog(){
  ...
  const choices=[...new Map([...Object.entries(PARTS).map(([key,p])=>[key,p.name]),...workshop.level.bodies.map((p,i)=>[p.id,`${PARTS[p.kind].name} #${i+1}`]),...workshop.level.inventory.map(entry=>[entry.id,`${PARTS[entry.part.kind].name} from bin`])])],select=$('goal-target'),hitBy=$('goal-hit-by');select.replaceChildren();hitBy.replaceChildren();for(const [value,label] of [['any','Any moving object'],...choices]){const option=document.createElement('option');option.value=value;option.textContent=label;select.append(option);}for(const [value,label] of choices){const option=document.createElement('option');option.value=value;option.textContent=label;hitBy.append(option);}
  const g=workshop.level.goal??{kind:'region',target:workshop.selected??'any',count:1,delay:0,edge:'right',state:'rung'};for(const k of ['kind','target','count','delay','edge','state'])$('goal-'+k).value=g[k];for(const option of hitBy.options)option.selected=(g.hitBy??[]).includes(option.value);syncGoalForm();closePopovers();$('goal-dialog').showModal();
}
function syncGoalForm(){const state=$('goal-kind').value==='state',impact=state&&['rung','on','popped'].includes($('goal-state').value);$('edge-row').hidden=$('goal-kind').value!=='edge';$('state-row').hidden=!state;$('hit-by-row').hidden=!impact;$('apply-goal').textContent=$('goal-kind').value==='region'?'Draw region':'Set goal';}
...
$('goal-state').addEventListener('change',syncGoalForm);
$('goal-form').addEventListener('submit',e=>{e.preventDefault();act(()=>{const g={kind:$('goal-kind').value,target:$('goal-target').value,count:Number($('goal-count').value),delay:Number($('goal-delay').value),edge:$('goal-edge').value,state:$('goal-state').value,x:800,y:600,w:180,h:180},hitBy=[...$('goal-hit-by').selectedOptions].map(option=>option.value);if(g.kind==='state'&&['rung','on','popped'].includes(g.state)&&hitBy.length)g.hitBy=hitBy;$('goal-dialog').close();if(g.kind==='region'){setTool('goal');gestures.pendingGoal=g;}else workshop.setGoal(g);});});
