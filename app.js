...
function sync(){
  const running=workshop.running,puzzle=workshop.level.mode==='puzzle',editor=workshop.level.mode==='editor';
  setIcon($('play'),running?'pause':'play');$('play').setAttribute('aria-label',running?'Pause simulation':'Play simulation');$('mode-status').textContent=running?'LIVE':'PAUSED';canvas.dataset.phase=running?'running':'paused';
  $('mode-name').textContent=editor?'Puzzle editor':puzzle?workshop.level.name:'Workshop';$('undo').disabled=!workshop.history.length;$('redo').disabled=!workshop.future.length;$('step').disabled=running;
...
function renderTray(){
  ...
  const signature=JSON.stringify([entries,gestures.tool,gestures.stock]);if(signature===traySignature)return;traySignature=signature;$('parts-grid').replaceChildren();$('bin-label').textContent=puzzle?entries.reduce((sum,e)=>sum+e.count,0):'∞';
  ...
  for(const entry of entries){const button=document.createElement('button');button.className='part-card';button.dataset.kind=entry.kind;button.setAttribute('aria-label',`Choose ${PARTS[entry.kind].name}${entry.count===null?'':`, ${entry.count} left`}`);button.title=PARTS[entry.kind].name;button.disabled=entry.count===0;button.classList.toggle('selected',gestures.tool===entry.kind&&gestures.stock===(entry.stock??null));
    ...
    button.addEventListener('pointerup',e=>{if(!trayDrag)return;const d=trayDrag;trayDrag=null;if(d.moved&&d.ghost&&!view.inBin(e.clientX,e.clientY)){if(workshop.canPlace(d.ghost))act(()=>{workshop.insert(d.ghost);workshop.record(d.before);});else toast('Objects cannot overlap.');}else if(!d.moved)setTool(entry.kind,entry.stock);});button.addEventListener('pointercancel',()=>{trayDrag=null;});button.addEventListener('click',e=>{if(e.detail===0)setTool(entry.kind,entry.stock);});$('parts-grid').append(button);
  }
}
...
function replaceLevel(level,running=false){gestures.cancel();const before=workshop.capture();workshop.load(level,running);workshop.history=[before];gestures.tool='select';gestures.stock=null;particles=[];shownWin=false;$('win').hidden=true;accumulator=0;traySignature='';fit();closePopovers();sync();}
...
function frame(now){
  ...
  if(workshop.running){accumulator+=dt*Number($('speed').value);while(accumulator>=1/120){consume(stepWorld(workshop.world));accumulator-=1/120;}for(const p of particles){p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=300*dt;}particles=particles.filter(p=>p.life>0).slice(-300);}
...
document.addEventListener('keydown',e=>{
  ...
  const key=e.key.toLowerCase();if((e.ctrlKey||e.metaKey)&&key==='z'){e.preventDefault();act(()=>{gestures.cancel();workshop.undo(e.shiftKey);});return;}
