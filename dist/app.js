...
import { createWorld, stepWorld, clamp } from './physics.js';
...
const snapshot=()=>({name:state.name,number:state.number,parts:structuredClone(state.parts),annotated:state.annotated});
...
function save(){
  try{localStorage.setItem(STORAGE,JSON.stringify({version:1,...snapshot()}));$('edit-status').textContent='Saved in this browser';}
...
function selectedPart(){return state.parts.find(p=>p.id===state.selected);}
function sync(){
  ...
  if(p){$('selected-name').textContent=PARTS[p.type].name;$('selected-description').textContent=PARTS[p.type].description;$('angle').min=p.type==='domino'?-85:-180;$('angle').max=p.type==='domino'?85:180;$('angle').value=p.angle;$('angle-value').value=`${Math.round(p.angle)}°`;}
  for(const id of ['angle','rotate-button','duplicate-button','delete-button'])$(id).disabled=state.phase!=='build';
  ...
  $('canvas-hint').textContent=running?'Click a marble to give it a nudge':paused?'Paused · Reset to rearrange your parts':'Drag to arrange · R to rotate · Space to run';
}
function addPart(type,x=550,y=240){
  if(!PARTS[type])throw Error('Unknown part type.');
  if(state.phase!=='build'){toast('Reset the machine to add or rearrange parts.');return false;}
  if(state.parts.length>=150){toast('The parts box holds 150 pieces per machine.');return false;}
  checkpoint();const offset=state.parts.filter(p=>Math.abs(p.x-x)<5&&Math.abs(p.y-y)<5).length*24;
  ...
  state.parts.push(p);state.selected=p.id;editDone();return p;
}
function updateSelected(change){if(state.phase!=='build')return;const p=selectedPart();if(!p)return;checkpoint();Object.assign(p,change);editDone();}
function removeSelected(){if(!selectedPart()||state.phase!=='build')return;checkpoint();state.parts=state.parts.filter(p=>p.id!==state.selected);state.selected=null;editDone();}
function duplicateSelected(){const p=selectedPart();if(!p||state.phase!=='build')return;if(state.parts.length>=150){toast('The workbench is full.');return;}checkpoint();const duplicate={...p,id:crypto.randomUUID(),x:clamp(p.x+35,20,WORLD.width-20),y:clamp(p.y+10,35,WORLD.floor-35)};state.parts.push(duplicate);state.selected=duplicate.id;editDone();}
function rotate(){const p=selectedPart();if(p)updateSelected({angle:p.type==='domino'?(p.angle+15>85?-85:p.angle+15):((p.angle+15+180)%360)-180});}
function undo(redo=false){const from=redo?state.future:state.history,to=redo?state.history:state.future;if(!from.length)return;reset();to.push(snapshot());Object.assign(state,from.pop());state.selected=null;save();sync();}
async function toggleRun(){
  if(!state.parts.some(p=>p.type==='marble')){toast('Add a marble to set things in motion.');return;}
...
canvas.addEventListener('pointerdown',event=>{
  ...
  if(state.phase!=='build'){
    if(state.phase==='running'&&p?.type==='marble'){p.vx+=220;p.vy-=150;sound.play({kind:'marble',strength:120});toast('A helpful little nudge.');}
    else if(p)toast('Reset the machine to rearrange parts.');return;
  }
...
canvas.addEventListener('pointermove',event=>{
  ...
  p.x=snap(clamp(point.x-drag.dx,20,WORLD.width-20));p.y=snap(clamp(point.y-drag.dy,30,WORLD.floor-15));
});
...
function draw(frameTime){
  ...
  for(const p of world?.parts??state.parts){drawPart(brush,p,time,state.phase==='running');if(p.id===state.selected&&state.phase==='build')drawSelection(brush,p);}
...
let angleBefore=null;$('angle').addEventListener('input',()=>{const p=selectedPart();if(!p)return;angleBefore??=snapshot();p.angle=Number($('angle').value);$('angle-value').value=`${p.angle}°`;});$('angle').addEventListener('change',()=>{if(angleBefore){checkpoint(angleBefore);angleBefore=null;editDone();}});
